...
//Gera a acurácia das predições com uma matriz de confusão
void getAccuracy(Graph *graph, double *acuracia){
  float tp = 0, fp = 0, tn = 0, fn = 0;
  int countSetosa = 0, countNoSetosa = 0;
  ...
  for(int i = 0; i < graph->qtdVertices; i++){
    ...
    for (int j = 0; j < currentVertex->deg; j++) {
      Vertex *edge = currentVertex->edges[j];
      if(edge->id <= 50)
        countSetosa++;
      else
        countNoSetosa++;
    }
    if(currentVertex->id <= 50){
      if(countSetosa >= countNoSetosa)
        tp++;
      else
        fn++;   
    }
    else{
      if(countNoSetosa > countSetosa)
        tn++;
      else
        fp++;
    }
  }
  *acuracia = (tp + tn) / (tp+fp+tn+fn) * 100;
}